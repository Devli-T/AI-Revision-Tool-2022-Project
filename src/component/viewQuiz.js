...
import { API_getQuestion, API_getQuestionsCount, sendPostRequest } from '../utils/apiUtils';
...
function ViewQuiz() {
    ...
    async function getQuestionCount(username, subject) {
        let count = await API_getQuestionsCount(username, subject);
    ...
    async function getQuestion(username, subject, questionNumber) {
        let questionAndAnswer = await API_getQuestion(username, subject, questionNumber);
